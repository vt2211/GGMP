...
try:
    ...
    _FVGP_IMPORT_ERROR = None
except Exception as exc:
    GP = None
    _FVGP_IMPORT_ERROR = exc


def _require_fvgp() -> None:
    if GP is None:
        raise ImportError(
            "GGMP requires optional dependency 'fvgp'. "
            "Install it (e.g. `pip install fvgp`) before constructing GGMP."
        ) from _FVGP_IMPORT_ERROR
...
def zero_mean(x, hyperparameters):
    """Zero mean function."""
    return np.zeros(len(x))

def linear_mean(x, hyperparameters):
...
def gaussian(mean, std, x):
    ...
    if not np.isfinite(inte) or inte < 1e-300:
        inte = 1e-300
    gn = g / inte
    if np.any(np.isnan(gn)):
        logging.warning("NaN in Gaussian normalized")
    return gn
...
class GGMP():
    def __init__(
        ...
        *,
        ...
        if gp_kernel_functions is None:
            self.gp_kernel_functions = [None] * self.number_of_GPs
        # Default to constant_mean function so mean hyperparameter gradient is computed correctly
        if gp_mean_functions is None:
            self.gp_mean_functions = [constant_mean] * self.number_of_GPs
    ...
    def initLikelihoods(self, init_mean = None, init_std = None, weights = None):
        ...
        if isinstance(init_mean, list):
            assert len(init_mean) == self.likelihood_terms
        if isinstance(init_std, list):
            assert len(init_std) == self.likelihood_terms
    ...
    def initGPs(self):
        ...
        if using_gpu:
            ...
            if n_gpu > 0:
                ...
                logging.warning(
                    "[initGPs] GPU compute was requested via gp_init_kwargs, "
                    "but no GPU backend was detected; falling back to CPU."
                )
                using_gpu = False
                self._gpu_engine = None
                self._effective_gp_device_ids = None
        ...
        def _make_component_gp(k):
            ...
            if gp_init_params is not None:
                kernel_keys = [key for key in kernel_keys if key in gp_init_params] + [key for key in kernel_keys if key not in gp_init_params]
            ...
            if gp_init_params is not None:
                mean_keys = [key for key in mean_keys if key in gp_init_params] + [key for key in mean_keys if key not in gp_init_params]
        ...
        logging.info("[initGPs] Synced hps_obj: mean values = %s", [float(h[-1]) for h in synced_hps])

        if dropped_keys_total:
            logging.info("[initGPs] Note: GP.__init__ rejected kwargs: %s", sorted(dropped_keys_total))
        ...
        if self._effective_gp_device_ids is not None and len(self._effective_gp_device_ids) > 1:
            logging.info(
                "[initGPs] Multi-GPU enabled: device_ids=%s (engine=%s)",
                self._effective_gp_device_ids,
                self._gpu_engine or "auto",
            )
        elif self._effective_gp_device_ids is not None:
            logging.info(
                "[initGPs] GPU enabled: device_id=%d (engine=%s)",
                self._effective_gp_device_ids[0],
                self._gpu_engine or "auto",
            )
...
class hyperparameters():
    ...
    def __init__(self, weights, weights_bounds,hps,hps_bounds):
        ...
        if len(hps) != len(hps_bounds):
            raise Exception("hps and hps_bounds have to be lists of equal length")
    ...
    def set(self,weights,hps):
        if len(hps) != len(self.hps_bounds):
            raise Exception("hps and hps_bounds have to be lists of equal length")
    ...
    *,
    ...
    for it in range(int(max_iter)):
        ...
        if int(log_every) > 0 and ((it % int(log_every) == 0) or it == int(max_iter) - 1):
            logging.info("[EM-MV %03d] obj=%.6f | L1_delta=%.3e | w=%s", it, obj, delta, np.array2string(w_new, precision=6))
...
def weight_objective_density(w: np.ndarray, terms) -> float:
    ...
    *,
    ...
    for t in range(int(max_iter)):
        ...
        if log_every and ((t % int(log_every) == 0) or t == int(max_iter) - 1):
            logging.info("[EM %03d] obj=%.6f | L1_delta=%.3e | w=%s", t, obj, delta, np.array2string(w_new, precision=6))
